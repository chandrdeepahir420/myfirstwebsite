...
// ==========================================
// 🚀 MULTI-CHANNEL OTP ENGINES
// ==========================================
async function sendEmailOTP(otp) {
    if (!process.env.GMAIL_EMAIL) return; 
    const mailOptions = {
        from: `"TeleDrive Security" <${process.env.GMAIL_EMAIL}>`,
        to: process.env.GMAIL_EMAIL, // Sends to your own email
        subject: "TeleDrive Login OTP",
        html: `<h3>TeleDrive Security</h3><p>Your login OTP is:</p><h1 style="color: #34c759; letter-spacing: 5px;">${otp}</h1>`
    };
    return transporter.sendMail(mailOptions).catch(e => console.log('Email Error'));
}

async function sendSmsOTP(otp) {
    if (!twilioClient || !process.env.ADMIN_PHONE || !process.env.TWILIO_PHONE_NUMBER) return;
    return twilioClient.messages.create({
        body: `Your TeleDrive OTP is: ${otp}.`,
        from: process.env.TWILIO_PHONE_NUMBER,
        to: process.env.ADMIN_PHONE
    }).catch(e => console.log('SMS Error'));
}

async function sendWhatsAppOTP(otp) {
    if (!twilioClient || !process.env.ADMIN_PHONE || !process.env.TWILIO_WHATSAPP_NUMBER) return;
    return twilioClient.messages.create({
        body: `🔐 *TeleDrive Security*\n\nYour login OTP is: *${otp}*`,
        from: process.env.TWILIO_WHATSAPP_NUMBER,
        to: `whatsapp:${process.env.ADMIN_PHONE}`
    }).catch(e => console.log('WhatsApp Error'));
}
...
// 1. Request OTP (Sends to Telegram, Email, SMS, WhatsApp)
app.post('/request-otp', async (req, res) => {
    try {
        ...
        if (process.env.TELEGRAM_BOT_TOKEN) {
            tasks.push(axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, { chat_id: process.env.TELEGRAM_CHAT_ID, text: `🔐 OTP: *${code}*`, parse_mode: 'Markdown' }).catch(e=>console.log('TG error')));
        }
        // Email, SMS, WhatsApp
        tasks.push(sendEmailOTP(code));
...
app.post('/api/vault/verify', (req, res) => {
    ...
    if (!masterPin) return res.json({ success: false, message: 'Server error: VAULT_PIN missing in Render Environment Variables!' });
    if (pin === masterPin) {
...
// ── 100% SAFE DOWNLOAD & PREVIEW ENGINE (STREAMING) ──
app.get('/download/:id', checkAuth, async (req, res) => {
    try {
        ...
        const errorMsg = e.response && e.response.data && e.response.data.description ? e.response.data.description : e.message;
        res.status(500).send(`
            <div style="font-family:sans-serif; text-align:center; margin-top:50px; background:#1e1e2a; color:white; padding:30px; border-radius:12px; max-width:400px; margin-left:auto; margin-right:auto;">
                <h2 style="color:#ff6584;">Download Failed ❌</h2>
                <p style="color:#e8e8f0;"><b>Reason:</b> ${errorMsg}</p>
...
app.get('/files', checkAuth, async (req, res) => {
    try {
        ...
        if(folderId === 'root') filter.$or = [{ folderId: 'root' }, { folderId: { $exists: false } }, { folderId: null }];

        const files = await FileModel.find(filter).sort({ uploadedAt: -1 }).skip(parseInt(req.query.skip) || 0).limit(parseInt(req.query.limit) || 100);
        res.json(files);
...
app.delete('/files/:id/trash', checkAuth, async (req, res) => {
    try {
        ...
        if (fileData.messageId) {
            try { await axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/deleteMessage`, { chat_id: process.env.TELEGRAM_CHAT_ID, message_id: fileData.messageId }); } catch (tgErr) {}
        }
...
app.delete('/folders/:id/trash', checkAuth, async (req, res) => {
    try {
        const updatedFolder = await FolderModel.findByIdAndUpdate(req.params.id, { isTrashed: true, trashedAt: new Date() }, { new: true });
...
app.delete('/files/:id/permanent', checkAuth, async (req, res) => {
    try {
        ...
        if(file && file.messageId) { 
            try { await axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/deleteMessage`, { chat_id: process.env.TELEGRAM_CHAT_ID, message_id: file.messageId }); } catch(err){} 
        }
...
setInterval(async () => {
    try {
        ...
        for(let file of expiredFiles) {
            if (file.messageId) { 
                try { await axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/deleteMessage`, { chat_id: process.env.TELEGRAM_CHAT_ID, message_id: file.messageId }); } catch(e){} 
            }
