...
async function searchFiles() {
    ...
    try {
        const res = await fetch('/files/all', { headers: getHeaders() });
...
async function verifyOTP() {
    ...
    try {
        ...
        const data = await res.json();
        if(data.success) { localStorage.setItem('td_token', data.token); sessionStorage.setItem('td_auth', 'true'); window.location.reload(); }
...
function switchView(view) {
    ...
    if(view === 'drive') { 
        document.getElementById('navDrive').classList.add('active'); navigateTo('root', 'My Drive'); 
    } else if(view === 'trash') { 
        document.getElementById('navTrash').classList.add('active'); loadTrash(); document.getElementById('breadcrumb').innerText = "Trash Bin"; 
    } else if(view === 'settings') { 
        document.getElementById('navSettings').classList.add('active'); document.getElementById('breadcrumb').innerText = "Security Settings"; document.getElementById('emptyState').style.display='none'; 
    }
    if(document.getElementById('sidebar').classList.contains('open')) toggleSidebar();
}
...
function renderItems(folders, files, isTrash) {
    ...
    folders.forEach(f => {
        ...
        d.addEventListener('click', (e) => { 
            ...
            else if(!isTrash) { navigateTo(f._id, f.name); }
        });
...
function previewFile(file) {
    const token = localStorage.getItem('td_token');
    const tokenUrl = token ? `/download/${file._id}?token=${token}` : file.url || `/download/${file._id}`;
